...
export function ResetPasswordClient({ token }: ResetPasswordClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) {
      setError('Reset token is missing or invalid. Please request a new OTP.');
      return;
    }
  ...
  return (
    ...
    >
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold mb-3" style={{ color: '#421C00' }}>
            Reset Password
          </h1>
          <p className="text-sm px-4" style={{ color: '#8E6545' }}>
            Create a new password with at least 8 characters, including uppercase, lowercase,
